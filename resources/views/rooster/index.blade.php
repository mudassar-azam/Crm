...
<div class="data-container datatable datatable-container">
    <button class="cview-button" onclick="window.location='{{ route('rooster.calendar.view') }}'">Calendar View</button>
            ...
            <tbody>
                @foreach($roosters as $rooster)
                    <tr id="row_{{ $rooster->id }}">
                        <td>{{ $rooster->user->user_name ?? '-' }}</td>
                        <td>{{ $rooster->type }}</td>
                        <td>{{ \Carbon\Carbon::parse($rooster->start_date)->format('d M Y')}}</td>
                        <td>{{ \Carbon\Carbon::parse($rooster->end_date)->format('d M Y')}}</td>
                        <td>
                            <button class="rounded-circle btn my-btn-primary btn-sm" type="button"
                                onclick="openEditPage({{ $rooster->id }})">
...
<script>
    ...
    $(document).on('click', '.delete-rooster', function() {
        ...
        if (confirm('Are you sure you want to delete this rooster?')) {
            $.ajax({
                url: '/rooster/destroy/' + roosterId,
                type: 'POST',
                headers: {
                ...
                success: function(response) {
                    ...
                    alert('Rooster deleted successfully');
                },
